...
def requires_connection(fn):
    """Ensures decorated method obect's exposes a valid connection object

    Any object pretending to talk with amazon simple workflow webservice
    should expose a valid `connection` attribute. See swf.base.Connexion.
    ...
    """
    def wrapped(self):
        if not hasattr(self, '_connection') or not self.connection:
            err_msg = "Missing AWS credentials, please set connection "\
